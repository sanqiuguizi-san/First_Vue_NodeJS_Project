module.exports = app => {
    ...
    router.get('/news/list', async (req, res) => {
        ...
        //聚合查询
        const cats = await Category.aggregate([{ //$执行一次操作，找出出以新闻分类为父类型的数据
                $match: {
                    parent: parent._id
                }
            },
            { //在上面的操作上再执行一个操作，注意此处的表是要跟数据库的表名一致
                $lookup: {
                    from: 'articles',
                    localField: '_id',
        ...
        //此时已获取到全部的分类了，需要根据各分类删除各自文章中其他分类的属性
        cats.map(cat => {
